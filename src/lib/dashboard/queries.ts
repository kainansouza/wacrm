...
export async function loadActivity(db: DB, limit = 20): Promise<ActivityItem[]> {
  ...
  for (const c of (contacts.data ?? []) as Array<{ id: string; name: string | null; phone: string; created_at: string }>) {
    items.push({
      ...
      textKey: 'activity.newContact',
      textVars: { name: c.name || c.phone },
      at: c.created_at,
      href: '/contacts',
    })
  }
  ...
  for (const d of (deals.data ?? []) as unknown as Array<{
    ...
    items.push({
      ...
      textKey: stage?.name ? 'activity.dealInStage' : 'activity.dealUpdated',
      textVars: stage?.name
  ...
  for (const b of (broadcasts.data ?? []) as Array<{
    ...
    const broadcastKey = b.status === 'sent' ? 'activity.broadcastSent' : 'activity.broadcastStatus'
    items.push({
      id: `broadcast-${b.id}`,
      kind: 'broadcast',
      text: `Broadcast "${b.name}"`,
      textKey: broadcastKey,
      textVars: {
        name: b.name || '',
        count: b.total_recipients,
        ...(b.status !== 'sent' ? { status: b.status } : {}),
      },
      at: b.created_at,
      href: '/broadcasts',
    })
  ...
  for (const l of (autoLogs.data ?? []) as unknown as Array<{
    ...
    items.push({
      id: `auto-${l.id}`,
      kind: 'automation',
      text: `Automation "${autoName}" ${isFailed ? 'failed for' : 'triggered for'} ${who}`,
      textKey: isFailed ? 'activity.automationFailed' : 'activity.automationTriggered',
      textVars: {
        name: autoName,
        who,
      },
