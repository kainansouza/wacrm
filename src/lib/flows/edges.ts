...
export interface CanvasEdge {
  ...
  label?: string;
  /** i18n key for the label, when it's a static value. */
  labelKey?: string;
}
...
export function outgoingSlots(node: BuilderNode): OutgoingSlot[] {
  ...
  switch (node.node_type) {
    ...
    case "collect_input":
    case "set_tag":
      return [{ id: "next", label: "Next", labelKey: "flowEdge.next" }];

    case "condition":
      return [
        { id: "true", label: "true", labelKey: "flowEdge.true" },
        { id: "false", label: "false", labelKey: "flowEdge.false" },
      ];
