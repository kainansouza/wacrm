...
/** Get the i18n labelKey for the formatLastSeen result, based on the time difference. */
export function getLastSeenKey(
  lastSeenAt: string | null | undefined,
  now: number,
): string {
  if (!lastSeenAt) return "presenceLabels.aWhileAgo";
  const last = new Date(lastSeenAt).getTime();
  if (Number.isNaN(last)) return "presenceLabels.aWhileAgo";

  const diff = Math.max(0, now - last);
  const mins = Math.floor(diff / 60_000);
  if (mins < 1) return "presenceLabels.justNow";
  if (mins === 1) return "presenceLabels.oneMinuteAgo";
  if (mins < 60) return "presenceLabels.minutesAgo";

  const hours = Math.floor(mins / 60);
  if (hours === 1) return "presenceLabels.oneHourAgo";
  if (hours < 24) return "presenceLabels.hoursAgo";

  const days = Math.floor(hours / 24);
  if (days === 1) return "presenceLabels.oneDayAgo";
  return "presenceLabels.daysAgo";
}
...
export function getPresenceLabelKey(
  status: PresenceStatus,
): string {
  switch (status) {
    case "online":
      return "presenceLabels.onlineActive";
    case "away":
      return "presenceLabels.awayIdle";
    case "offline":
      return "presenceLabels.offlineLastSeen";
  }
}
