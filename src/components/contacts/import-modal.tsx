      ...
      {tagNames.map((name) => {
        ...
        return (
          <span
            ...
            title={isKnown ? name : t('contacts.importModal.tagWillBeCreated', { name })}
          >
            <span
...
export function ImportModal({
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (rows.length === 0) {
      toast.error(t('toast.noValidRowsFound'));
      setParsedRows([]);
      setHasTagsColumn(false);
      setHasCompanyColumn(false);
      setTagColorByKey(new Map());
  ...
  async function handleImport() {
    ...
    try {
      ...
      try {
        ...
        toast.warning(t('toast.failedImportTagAssignments'));
      }

      setResult({ imported, skipped, failed, tagsAssigned });
      if (imported > 0) {
        toast.success(t('toast.contactsImported', { count: imported }));
        onImported();
      }
      if (tagsAssigned > 0) {
        toast.success(t('toast.tagsAssigned', { count: tagsAssigned }));
      }
      if (skippedNames.length > 0) {
        const sample = skippedNames.slice(0, 3).join(', ');
        const more = skippedNames.length > 3 ? ` (+${skippedNames.length - 3} more)` : '';
        toast.info(t('toast.unknownTagsSkipped', { names: `${sample}${more}` }));
      }
      if (skipped > 0) {
        toast.info(t('toast.duplicatesSkipped', { count: skipped }));
      }
      if (failed > 0) {
        toast.error(t('toast.contactImportFailed', { count: failed }));
      }
    } catch (err: unknown) {
        ...
        <div className="shrink-0 space-y-4 border-b border-border/80 px-6 pt-6 pb-5">
          <DialogHeader className="gap-1.5">
            ...
            <DialogDescription className="leading-relaxed text-muted-foreground">
              {t('contacts.importModal.uploadDescription')}
            </DialogDescription>
          </DialogHeader>
            ...
            {file ? (
              <>
                ...
                <span className="rounded-full bg-muted px-2.5 py-0.5 text-[11px] font-medium text-muted-foreground">
                  {t('contacts.importModal.rowsReady', { N: parsedRows.length })}
                </span>
              </>
            ) : (
              <>
                ...
                <p className="text-[11px] text-muted-foreground">
                  {t('contacts.importModal.csvFormat')}
                </p>
              </>
            )}
          </div>
            ...
            <div className="space-y-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <p className="text-[11px] font-semibold tracking-[0.14em] text-muted-foreground uppercase">
                  {t('contacts.importModal.preview', { N: preview.length })}
                </p>
              ...
              {parsedRows.length > PREVIEW_LIMIT && (
                <p className="text-center text-[11px] text-muted-foreground">
                  {t('contacts.importModal.moreRows', { N: parsedRows.length - PREVIEW_LIMIT })}
                </p>
              )}
            </div>
          )}
              ...
              <div className="mt-3 flex flex-wrap gap-3">
                {result.imported > 0 && (
                  <div className="text-primary flex items-center gap-1.5 text-sm">
                    ...
                    {t('contacts.importModal.resultImported', { N: result.imported })}
                  </div>
                )}
                {result.tagsAssigned > 0 && (
                  <div className="flex items-center gap-1.5 text-sm text-cyan-400">
                    <CheckCircle className="size-4 shrink-0" />
                    {t('contacts.importModal.resultTagsAssigned', { N: result.tagsAssigned })}
                  </div>
                )}
                {result.skipped > 0 && (
                  <div className="flex items-center gap-1.5 text-sm text-amber-400">
                    <AlertTriangle className="size-4 shrink-0" />
                    {t('contacts.importModal.resultSkipped', { N: result.skipped })}
                  </div>
                )}
                {result.failed > 0 && (
                  <div className="flex items-center gap-1.5 text-sm text-red-400">
                    <XCircle className="size-4 shrink-0" />
                    {t('contacts.importModal.resultFailed', { N: result.failed })}
                  </div>
                )}
              </div>
            </div>
        ...
        <DialogFooter className="mt-0 shrink-0 gap-2 border-t border-border/80 bg-background/50 px-6 py-4 sm:justify-end">
          ...
          {!result && (
            ...
            >
              ...
              {t('contacts.importModal.importButton', { N: parsedRows.length })}
            </Button>
          )}
        </DialogFooter>
      </DialogContent>
