...
export function CustomFieldsPanel() {
  ...
  async function handleCreate() {
    ...
    if (isDuplicate(name)) {
      toast.error(t('toast.duplicateField', { name }));
      return;
    }
    ...
    if (error) {
      toast.error(t('toast.couldNotCreateField'));
      return;
    }
    toast.success(t('toast.fieldCreated', { name }));
    setNewName('');
    await fetchFields();
  }
  ...
  async function handleRename(
    ...
    setBusyId(null);
    if (error) {
      toast.error(t('toast.couldNotRenameField'));
      return false;
    }
    await fetchFields();
    return true;
  ...
  async function handleDelete(field: CustomField) {
    if (
      !window.confirm(t('contacts.customFields.deleteConfirm', { field: field.field_name }))
    ) {
      return;
    }
    setBusyId(field.id);
    ...
    setBusyId(null);
    if (error) {
      toast.error(t('toast.couldNotDeleteField'));
      return;
    }
    toast.success(t('toast.fieldDeleted', { name: field.field_name }));
    await fetchFields();
  }
  ...
  return (
    <li className="flex items-center gap-2 px-3 py-2">
      <Input
        ...
        aria-label={t('contacts.customFields.renameField', { field: field.field_name })}
      ...
      <Button
        ...
        title={t('contacts.customFields.deleteField', { field: field.field_name })}
