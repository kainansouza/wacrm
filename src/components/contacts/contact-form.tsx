...
export function ContactForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!isEdit && dupMatch?.exact) {
      toast.error(t('toast.contactAlreadyExists'));
      return;
    }
    ...
    try {
      ...
      toast.success(isEdit ? t('toast.contactUpdated') : t('toast.contactCreated'));
      onOpenChange(false);
      onSaved();
    } catch (err: unknown) {
      ...
      const message = err instanceof Error ? err.message : t('toast.failedSaveContact');
      toast.error(message);
    } finally {
      setSaving(false);
    }
              ...
              >
                ...
                <div className="space-y-1">
                  ...
                  {onViewExisting && (
                    ...
                    >
                      {t('contacts.form.viewExisting', { name: dupMatch.contact.name || dupMatch.contact.phone })}
                    </button>
                  )}
                </div>
              </div>
