...
export function MessageComposer({
  ...
  const finalizeRecording = useCallback(
    async (bytes: Uint8Array) => {
      ...
      if (file.size > MEDIA_MAX_BYTES_BY_KIND.audio) {
        toast.error(t('toast.recordingTooLong'));
        return;
      }
      setBusy(true);
      try {
  ...
  const startRecording = useCallback(async () => {
    ...
    if (!navigator.mediaDevices?.getUserMedia || typeof AudioContext === "undefined") {
      toast.error(t('toast.voiceNotSupported'));
      return;
    }
    try {
    ...
    <div className="border-t border-border bg-card p-3">
      ...
      {sessionExpired && (
        <div className="mb-2 flex items-center justify-between rounded-lg bg-amber-500/10 px-3 py-2">
          <p className="text-xs text-amber-400">
            {t('inbox.messageComposer.sessionExpiredTooltip')}
          </p>
          <Button
            variant="ghost"
            size="sm"
          ...
          >
            ...
            {t('inbox.messageComposer.templates')}
          </Button>
        </div>
      )}
      ...
      {draft ? (
        ...
        <div className="flex items-center gap-3 rounded-xl border border-border bg-muted px-4 py-2.5">
          ...
          <span className="flex-1 text-sm text-foreground">
            {t('inbox.messageComposer.recording')} {formatDuration(recordSeconds)} /{" "}
          ...
          >
            {t('inbox.messageComposer.cancel')}
          </button>
          <Button
            size="sm"
            onClick={stopRecording}
            className="h-9 w-9 shrink-0 bg-primary p-0 hover:bg-primary/90"
            title={t('inbox.messageComposer.stopAndAttach')}
          >
        ...
        <div className="flex items-end gap-2">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger
              ...
              title={
                readOnly
                  ? t('inbox.messageComposer.readOnlyViewer')
                  : inputsDisabled
                    ? undefined
                    : t('inbox.messageComposer.attachMedia')
              }
            ...
            <DropdownMenuContent align="start" className="border-border bg-popover">
              <DropdownMenuItem onClick={() => imageInputRef.current?.click()}>
                ...
                {t('inbox.messageComposer.photo')}
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => videoInputRef.current?.click()}>
                <Video className="mr-2 h-4 w-4" />
                {t('inbox.messageComposer.video')}
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => documentInputRef.current?.click()}>
                <FileText className="mr-2 h-4 w-4" />
                {t('inbox.messageComposer.document')}
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => void startRecording()}>
                <Mic className="mr-2 h-4 w-4" />
                {t('inbox.messageComposer.voiceNote')}
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
          ...
          <GatedButton
            ...
            title={readOnly ? undefined : t('inbox.messageComposer.sendTemplate')}
          ...
          <textarea
            ...
            placeholder={
              readOnly
                ? t('inbox.messageComposer.readOnlyViewer')
                : sessionExpired
                  ? t('inbox.messageComposer.sessionExpired')
                  : t('inbox.messageComposer.placeholder')
            }
            ...
            title={readOnly ? t('inbox.messageComposer.readOnlyViewer') : undefined}
            className={cn(
      ...
      {!draft && !recording && (
        <p className="mt-1 pl-[5.5rem] text-[10px] text-muted-foreground">
          {t('inbox.messageComposer.quickRepliesTooltip')}
        </p>
      )}
    </div>
  );
      ...
      <div className="mt-2 flex items-end gap-2">
        {draft.kind !== "audio" && (
          <input
            ...
            placeholder={t('inbox.messageComposer.addCaption')}
