...
function formatDateSeparator(dateStr: string, t: ReturnType<typeof useT>['t']): string {
...
function getStatusOptions(t: ReturnType<typeof useT>['t']) {
  return [
    { label: t('inbox.conversationList.open'), value: "open" as ConversationStatus, color: "text-primary" },
    { label: t('inbox.conversationList.pending'), value: "pending" as ConversationStatus, color: "text-amber-400" },
    { label: t('inbox.conversationList.closed'), value: "closed" as ConversationStatus, color: "text-muted-foreground" },
  ];
}
...
export function MessageThread({
  ...
  const sessionInfo = useMemo(() => {
    ...
    if (!lastCustomerMsg) return { expired: true, remaining: t('inbox.messageThread.noCustomerMessages') };

    const hoursSince = differenceInHours(new Date(), new Date(lastCustomerMsg.created_at));
    const expired = hoursSince >= 24;

    if (expired) {
      return { expired: true, remaining: t('inbox.messageThread.sessionExpired') };
    }

    const hoursLeft = 24 - hoursSince;
    const remaining =
      hoursLeft >= 1
        ? t('inbox.messageThread.sessionRemainingHours', { X: Math.floor(hoursLeft) })
        : t('inbox.messageThread.sessionRemainingMinutes', { X: Math.floor(hoursLeft * 60) });
    ...
    async (text: string, replyToId?: string) => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          toast.error(t('toast.failedToSend', { reason }));
    ...
    async (
      ...
      try {
        ...
        if (!res.ok) {
          ...
          toast.error(t('toast.failedToSendTemplate', { reason }));
          onUpdateMessage(tempId, { status: "failed" });
          return;
        }
  ...
  const contactDisplayName = contact?.name || contact?.phone || t('inbox.messageThread.customer');
  ...
  const authorLabelFor = useCallback(
    (m: Message): string => {
      ...
      return isAgentMsg ? t('inbox.messageThread.you') : contactDisplayName;
    },
  ...
  const postReaction = useCallback(
    async (messageId: string, emoji: string) => {
      ...
      if (messageId.startsWith("temp-")) {
        toast.error(t('toast.waitForMessageToFinishSending'));
        return;
      }
      ...
      try {
        ...
        const reason = err instanceof Error ? err.message : "network error";
        toast.error(t('toast.reactionFailed', { reason }));
  ...
  const handleAssignChange = useCallback(
    async (agentId: string | null) => {
      ...
      if (error) {
        ...
        toast.error(t('toast.failedUpdateAssignment'));
        return;
      }
      ...
      <div className={cn("flex flex-1 flex-col items-center justify-center", DOODLE_BG_CLASSES)}>
        <div className="flex h-16 w-16 items-center justify-center rounded-full bg-muted">
          ...
          <h3 className="mt-4 text-sm font-medium text-muted-foreground">
            {t('inbox.messageThread.selectConversation')}
          </h3>
          <p className="mt-1 text-xs text-muted-foreground">
            {t('inbox.messageThread.selectConversationDescription')}
          </p>
      </div>
    );
  }
  ...
  const assignLabel = assignedAgentId
    ? (currentAssignee?.full_name ?? t('inbox.messageThread.assignedTo'))
    : t('inbox.messageThread.assign');
        ...
        <div className="flex min-w-0 items-center gap-2 sm:gap-3">
          ...
          {onBack && (
            <button
              type="button"
              onClick={onBack}
              aria-label={t('inbox.messageThread.backToConversations')}
        ...
        <div className="flex items-center gap-2">
          ...
          {onToggleContactPanel && (
            <button
              ...
              aria-label={
                contactPanelOpen ? t('inbox.messageThread.hideContact') : t('inbox.messageThread.showContact')
              }
              aria-pressed={contactPanelOpen}
              title={contactPanelOpen ? t('inbox.messageThread.hideContact') : t('inbox.messageThread.showContact')}
              className={cn(
          ...
          {onRefresh && (
            <button
              ...
              aria-label={t('inbox.messageThread.refresh')}
              title={t('inbox.messageThread.refresh')}
              className={cn(
          ...
          <DropdownMenu>
            <DropdownMenuTrigger className={cn(
                ...
                {currentStatus?.label ?? t('inbox.messageThread.status')}
            ...
            >
              {profiles.length === 0 ? (
                <DropdownMenuItem disabled className="text-sm text-muted-foreground">
                  {t('inbox.messageThread.noTeammatesAvailable')}
                </DropdownMenuItem>
              ) : (
                profiles.map((p) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="flex-1">
                        {p.full_name}
                        {p.user_id === user?.id ? t('inbox.messageThread.me') : ""}
                      </span>
              ...
              {assignedAgentId && (
                <>
                  ...
                  >
                    {t('inbox.messageThread.unassign')}
                  </DropdownMenuItem>
                </>
              )}
            </DropdownMenuContent>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12">
            <p className="text-sm text-muted-foreground">{t('inbox.messageThread.noMessages')}</p>
            <p className="text-xs text-muted-foreground">
              {t('inbox.messageThread.sendTemplateToStart')}
            </p>
          </div>
        ) : (
