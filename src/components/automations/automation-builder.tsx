...
const STEP_META: Record<AutomationStepType, StepMeta> = {
  send_message: { label: "", icon: MessageSquare, border: "border-l-primary" },
  send_template: { label: "", icon: FileText, border: "border-l-primary" },
  add_tag: { label: "", icon: Tag, border: "border-l-primary" },
  remove_tag: { label: "", icon: TagIcon, border: "border-l-primary" },
  assign_conversation: { label: "", icon: UserCheck, border: "border-l-primary" },
  update_contact_field: { label: "", icon: PencilLine, border: "border-l-primary" },
  create_deal: { label: "", icon: Briefcase, border: "border-l-primary" },
  wait: { label: "", icon: Hourglass, border: "border-l-border" },
  condition: { label: "", icon: GitBranch, border: "border-l-amber-500" },
  send_webhook: { label: "", icon: Webhook, border: "border-l-primary" },
  close_conversation: { label: "", icon: CircleSlash, border: "border-l-primary" },
}

const STEP_LABEL: Record<AutomationStepType, string> = {
  send_message: "automations.stepTypeSendMessage",
  send_template: "automations.stepTypeSendTemplate",
  add_tag: "automations.stepTypeAddTag",
  remove_tag: "automations.stepTypeRemoveTag",
  assign_conversation: "automations.stepTypeAssignConversation",
  update_contact_field: "automations.stepTypeUpdateField",
  create_deal: "automations.stepTypeCreateDeal",
  wait: "automations.stepTypeWait",
  condition: "automations.stepTypeCondition",
  send_webhook: "automations.stepTypeSendWebhook",
  close_conversation: "automations.stepTypeCloseConversation",
}
...
const TRIGGER_OPTIONS: { value: AutomationTriggerType; labelKey: string; hintKey: string }[] = [
  { value: "new_message_received", labelKey: "automations.triggerNewMessage", hintKey: "automations.triggerNewMessageHint" },
  {
    value: "first_inbound_message",
    labelKey: "automations.triggerFirstInbound",
    hintKey: "automations.triggerFirstInboundHint",
  },
  { value: "keyword_match", labelKey: "automations.triggerKeyword", hintKey: "automations.triggerKeywordHint" },
  { value: "new_contact_created", labelKey: "automations.triggerNewContact", hintKey: "automations.triggerNewContactHint" },
  { value: "conversation_assigned", labelKey: "automations.triggerAssigned", hintKey: "automations.triggerAssignedHint" },
  { value: "tag_added", labelKey: "automations.triggerTagAdded", hintKey: "automations.triggerTagAddedHint" },
  { value: "time_based", labelKey: "automations.triggerTimeBased", hintKey: "automations.triggerTimeBasedHint" },
]
  ...
  return (
    ...
    >
      ...
      {customFields.length > 0 && (
        <optgroup label={t('contacts.customFields.title')}>
          {customFields.map((f) => (
...
function SendTemplateFields({
  ...
  if (templates.length === 0) {
    return (
      <>
        <FieldBlock label={t('settings.templates.name')}>
          <Input
            value={templateName}
            onChange={(e) =>
        ...
        <FieldBlock label={t('settings.templates.language')}>
          <Input
            value={language}
            onChange={(e) =>
  ...
  return (
    <FieldBlock label={t('automations.selectTemplate')}>
      <select
        value={current}
        onChange={(e) => {
      ...
      >
        ...
        {current && !hasMatch && (
          <option value={current}>
            {t('automations.templateNotApproved', { name: templateName, language: language || t('common.unknown') })}
          </option>
        )}
      </select>
    </FieldBlock>
      ...
      if (!res.ok) {
        ...
        if (firstIssue?.message) {
          toast.error(firstIssue.message, {
            description: firstIssue.path ? `${t('automations.saveFailedAt')} ${firstIssue.path}` : undefined,
          })
        } else {
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="truncate text-sm font-medium text-foreground">
              {t(TRIGGER_OPTIONS.find((o) => o.value === type)?.labelKey ?? "") || type}
            </div>
          </div>
          <ChevronDown
          ...
          <div className="space-y-3 border-t border-border px-4 py-3">
            <div>
              ...
              <p className="mt-1 text-[11px] text-muted-foreground">
                {t(TRIGGER_OPTIONS.find((o) => o.value === type)?.hintKey ?? "")}
              </p>
            </div>
            {type === "keyword_match" && (
              <KeywordMatchConfig
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground">
                {isCondition ? t('automations.stepKindCondition') : step.step_type === "wait" ? t('automations.stepKindWait') : t('automations.stepKindAction')}
              </div>
              <div className="truncate text-sm font-medium text-foreground">{t(STEP_LABEL[step.step_type])}</div>
...
function ConditionBranches({
  ...
  return (
    ...
    <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
      <BranchColumn label={t('common.yes')} color="text-primary">
        <StepList {...props} steps={yes} parentPath={yesPath} />
      </BranchColumn>
      <BranchColumn label={t('common.no')} color="text-rose-400">
...
function StepEditor({
  ...
  switch (step.step_type) {
    case "send_message":
      return (
        <FieldBlock label={t('automations.stepEditorMessageText')}>
          <Textarea
            value={(cfg.text as string) ?? ""}
            onChange={(e) => set({ text: e.target.value })}
            placeholder={t('automations.stepEditorMessagePlaceholder')}
    ...
    case "remove_tag":
      return (
        <FieldBlock label={t('common.tags')}>
          <TagSelect
    ...
    case "assign_conversation":
      return (
        <>
          <FieldBlock label={t('automations.stepEditorMode')}>
            <select
            ...
            >
              ...
              <option value="specific">{t('automations.modeSpecific')}</option>
            </select>
          </FieldBlock>
          {cfg.mode === "specific" && (
            <FieldBlock label={t('automations.stepEditorAgent')}>
              <AgentSelect
    ...
    case "update_contact_field":
      return (
        <>
          <FieldBlock label={t('automations.stepEditorField')}>
            <ContactFieldSelect
              value={(cfg.field as string) ?? "name"}
              onChange={(v) => set({ field: v })}
            />
          </FieldBlock>
          <FieldBlock label={t('common.value')}>
            <Input
              value={(cfg.value as string) ?? ""}
              onChange={(e) => set({ value: e.target.value })}
              placeholder={t('automations.stepEditorFieldPlaceholder')}
              className="bg-muted text-foreground"
            />
          </FieldBlock>
        </>
    ...
    case "create_deal":
      return (
        <>
          <FieldBlock label={t('automations.stepEditorPipelineId')}>
            <Input
          ...
          <FieldBlock label={t('automations.stepEditorStageId')}>
            <Input
          ...
          <FieldBlock label={t('automations.stepEditorTitle')}>
            <Input
    ...
    case "wait":
      return (
        <div className="grid grid-cols-2 gap-2">
          <FieldBlock label={t('automations.stepEditorAmount')}>
            <Input
              type="number"
              min={1}
          ...
          <FieldBlock label={t('automations.stepEditorUnit')}>
            <select
    ...
    case "condition":
      return (
        <>
          <FieldBlock label={t('automations.stepEditorSubject')}>
            <select
              value={(cfg.subject as string) ?? "tag_presence"}
              onChange={(e) => set({ subject: e.target.value })}
              className="w-full rounded-md border border-border bg-muted px-2 py-1.5 text-sm text-foreground"
            >
              <option value="tag_presence">{t('automations.subjectTagPresence')}</option>
              <option value="contact_field">{t('automations.subjectContactField')}</option>
              <option value="message_content">{t('automations.subjectMessageContent')}</option>
              <option value="time_of_day">{t('automations.subjectTimeOfDay')}</option>
            </select>
          </FieldBlock>
          <FieldBlock label={t('automations.stepEditorOperand')}>
            <Input
              placeholder={
                cfg.subject === "time_of_day"
                  ? "HH:mm-HH:mm"
                  : cfg.subject === "contact_field"
                  ? t('automations.conditionOperandPlaceholder')
                  : ""
              }
    ...
    case "send_webhook":
      return (
        <>
          <FieldBlock label={t('automations.stepEditorUrl')}>
            <Input
          ...
          <FieldBlock label={t('automations.stepEditorBodyTemplate')}>
            <Textarea
    ...
    case "close_conversation":
      return (
        <p className="text-xs text-muted-foreground">
          {t('automations.stepEditorNoConfig')}
        </p>
      )
    default:
      return null
...
function previewFor(step: BuilderStep, t: ReturnType<typeof useT>["t"]): string {
  switch (step.step_type) {
    case "send_message":
      return (step.step_config.text as string) || t('automations.noTextYet')
    case "send_template":
      return (step.step_config.template_name as string) || t('automations.pickATemplate')
    case "wait":
      return `${step.step_config.amount ?? "?"} ${step.step_config.unit ?? ""}`
    case "condition":
      return t('automations.conditionSubjectWhen', { subject: (step.step_config.subject as string) ?? "?" })
    case "send_webhook":
