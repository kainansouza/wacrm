  ...
  return (
    <section className="flex h-full flex-col rounded-xl border border-border bg-card">
      <header className="flex items-center justify-between border-b border-border px-5 py-4">
        <div>
          <h2 className="text-sm font-semibold text-foreground">{t('chart.conversationsTitle')}</h2>
          <p className="mt-0.5 text-xs text-muted-foreground">{t('chart.conversationsDesc')}</p>
        </div>
      ...
      <div className="p-5">
        {loading || !data ? (
          ...
          <EmptyState
            icon={MessageSquare}
            title={t('chart.noMessageActivity')}
            hint={t('chart.messageActivityHint')}
          />
        ) : (
      ...
      <footer className="flex items-center gap-4 border-t border-border px-5 py-3 text-xs text-muted-foreground">
        <LegendDot color="#3b82f6" label={t('chart.incoming')} />
        <LegendDot color="#7c3aed" label={t('chart.outgoing')} />
      </footer>
    </section>
  )
}
