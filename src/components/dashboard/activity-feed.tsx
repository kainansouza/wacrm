      ...
      {loading || !items ? (
        ...
        <div className="p-5">
          <EmptyState
            ...
            hint={t('dashboard.noActivityDescription')}
          />
        </div>
      ) : (
        <>
              ...
              const row = (
                <div className="flex items-center gap-3 px-5 py-2.5">
                  ...
                  <span className="min-w-0 flex-1 truncate text-sm text-foreground">
                    {it.textKey ? t(it.textKey, it.textVars) : it.text}
                  </span>
          ...
          <footer className="flex items-center justify-between border-t border-border px-5 py-3 text-xs">
            <span className="text-muted-foreground tabular-nums">
              {t('dashboard.showingNofM', { N: visible.length, M: totalLoaded + (totalLoaded === 50 ? '+' : '') })}
            </span>
...
function relativeTime(iso: string, t: ReturnType<typeof useT>['t']): string {
  const then = new Date(iso).getTime()
  if (Number.isNaN(then)) return ''
  const diffSec = Math.round((Date.now() - then) / 1000)
  if (diffSec < 60) return t('dashboard.timeSecondsAgo', { N: Math.max(1, diffSec) })
  if (diffSec < 3600) return t('dashboard.timeMinutesAgo', { N: Math.floor(diffSec / 60) })
  if (diffSec < 86400) return t('dashboard.timeHoursAgo', { N: Math.floor(diffSec / 3600) })
  if (diffSec < 2_592_000) return t('dashboard.timeDaysAgo', { N: Math.floor(diffSec / 86400) })
