          ...
          <div>
            ...
            <p
              ...
              dangerouslySetInnerHTML={{
                __html: t("flows.interpolationHelp", {
                  var: (cfg as { var_key?: string }).var_key || "name",
                }),
              }}
            />
          </div>
          ...
          rows: [
            {
              ...
              title: `${t("flows.optionDefault")} ${totalRows + 1}`,
              next_node_key: "",
            },
          ],
        },
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <TextRow
          label={t("flows.tapToExpandLabel")}
  ...
  return (
    <>
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        <div>
          <label className="mb-1 block text-xs text-muted-foreground">{t("flows.ifLabel")}</label>
          <Select
            value={subject}
            onValueChange={(v) =>
        ...
        <div className="md:col-span-2">
          ...
          {subject === "tag" && tags.length > 0 ? (
            ...
            <Input
              ...
              placeholder={subject === "var" ? t("flows.varPlaceholder") : t("flows.tagUuidPlaceholder")}
              className="bg-muted font-mono text-xs"
            />
          )}
        </div>
  ...
  const handleFile = useCallback(
    async (file: File) => {
      if (file.size > MEDIA_MAX_BYTES) {
        toast.error(
          t("flows.fileTooBig", { size: (file.size / 1024 / 1024).toFixed(1) }),
        );
        return;
      }
      setUploading(true);
      try {
        ...
        const msg =
          err instanceof Error ? err.message : t("flows.uploadFailed");
        toast.error(msg);
      } finally {
        setUploading(false);
      }
    ...
    <>
      <div>
        ...
        >
          ...
          <SelectContent>
            <SelectItem value="image">{t("flows.imageLabel")}</SelectItem>
            <SelectItem value="video">{t("flows.videoLabel")}</SelectItem>
            <SelectItem value="document">
              {t("flows.documentLabel")}
            </SelectItem>
          </SelectContent>
        </Select>
      </div>
      ...
      <div>
        <label className="mb-1 block text-xs text-muted-foreground">{t("flows.fileLabel")}</label>
        {cfg.media_url ? (
