...
export function defaultConfigFor(
  ...
  switch (type) {
    ...
    case "send_list":
      return {
        text: "",
        button_label: t ? t("flows.viewOptions") : "View options",
        sections: [
          {
            title: "",
            rows: [
...
export function FlowEditorProvider({
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(json.error ?? t("flows.flowSaveFailedDetail", { status: res.status }));
      }
      setDirty(false);
      toast.success(t("flows.flowSaved"));
    } catch (err) {
      const msg = err instanceof Error ? err.message : t("flows.flowSaveFailed");
      toast.error(msg);
    } finally {
      setSaving(false);
    }
  ...
  const setStatus = useCallback(
    async (next: BuilderState["status"]) => {
      ...
      try {
        ...
        if (!res.ok) {
          const json = await res.json().catch(() => ({}));
          throw new Error(json.error ?? t("flows.statusUpdateFailedDetail", { status: res.status }));
        }
        setStateRaw((s) => ({ ...s, status: next }));
        toast.success(
          next === "active"
            ? t("flows.flowActivated")
            : next === "archived"
              ? t("flows.archived")
              : t("flows.savedAsDraft"),
        );
      } catch (err) {
  ...
  const deleteFlow = useCallback(async () => {
    const yes = window.confirm(t("flows.deleteConfirm", { name: state.name }));
    if (!yes) return;
    try {
      const res = await fetch(`/api/flows/${initialFlow.id}`, {
        method: "DELETE",
      });
      if (!res.ok) throw new Error(t("flows.deleteFailedDetail", { status: res.status }));
      router.push("/flows");
    } catch (err) {
