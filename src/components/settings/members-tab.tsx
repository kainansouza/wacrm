...
export function MembersTab() {
  ...
  const loadEverything = useCallback(async () => {
    try {
      ...
      if (!mres.ok) {
        ...
        toast.error(payload.error || t('toast.failedLoadMembers'));
        return;
      }
      ...
      if (ires) {
        if (!ires.ok) {
          ...
          toast.error(payload.error || t('toast.failedLoadInvitations'));
          return;
        }
      ...
      toast.error(t('toast.couldNotReachServer'));
    } finally {
      setLoading(false);
    }
  }, [canManageMembers]);
  ...
  async function handleRoleChange(member: Member, nextRole: AccountRole) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(payload.error || t('toast.failedUpdateRole'));
        return;
      }
      toast.success(t('toast.roleUpdated', { name: member.full_name || 'member', role: nextRole }));
    } catch (err) {
  ...
  async function handleRemove() {
    ...
    try {
      ...
      if (!res.ok) {
        const payload = await res.json().catch(() => ({}));
        toast.error(payload.error || t('toast.failedRemoveMember'));
        return;
      }
      toast.success(t('toast.memberRemoved', { name: removingMember.full_name || 'member' }));
      setMembers((prev) =>
  ...
  async function handleRevoke(invite: Invitation) {
    try {
      ...
      if (!res.ok) {
        const payload = await res.json().catch(() => ({}));
        toast.error(payload.error || t('toast.failedRevokeInvitation'));
        return;
      }
          ...
          return (
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
              ...
              <span className="text-muted-foreground/70">
                · {t('settings.overview.members', { N: members.length })}
              </span>
            </div>
          );
        })()}
                  ...
                  <div className="flex items-center gap-2 sm:gap-3">
                    ...
                    {canManageMembers && !isOwnerRow && !isSelf ? (
                      ...
                      >
                        ...
                        {t('settings.members.' + member.role)}
                      </span>
                    )}
