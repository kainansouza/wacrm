...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    if (trimmedLabel.length > MAX_LABEL_LEN) {
      toast.error(t('toast.labelTooLong', { max: MAX_LABEL_LEN }));
      return;
    }
    setSubmitting(true);
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(payload.error || t('toast.failedCreateInvitation'));
        return;
      }
      ...
      toast.error(t('toast.couldNotReachServerTryAgain'));
    } finally {
      setSubmitting(false);
    }
  }
    ...
    >
      <DialogContent className="bg-popover border-border sm:max-w-md">
        {result ? (
          <>
            <DialogHeader>
              ...
              <DialogDescription className="text-muted-foreground">
                {t('settings.members.inviteDialog.description')} They&apos;ll be able
                to sign up (or sign in) and join the account as{' '}
                <span className="font-medium text-muted-foreground">{t('settings.members.' + result.role)}</span>
            ...
            <div className="space-y-3 py-2">
              <Label className="text-muted-foreground">{t('settings.members.inviteDialog.title')}</Label>
              ...
              >
                ...
                {t('settings.members.inviteDialog.sendViaWhatsApp')}
              </a>
            </div>
          ...
          <>
            <DialogHeader>
              <DialogTitle className="text-popover-foreground">{t('settings.members.inviteDialog.title')}</DialogTitle>
              <DialogDescription className="text-muted-foreground">
            ...
            <div className="space-y-4 py-2">
              <div className="space-y-2">
                <Label className="text-muted-foreground">{t('settings.members.inviteDialog.roleLabel')}</Label>
                <Select
                  value={role}
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="agent">{t('settings.members.agent')}</SelectItem>
                    <SelectItem value="viewer">{t('settings.members.viewer')}</SelectItem>
                  </SelectContent>
                </Select>
                <p className="text-xs text-muted-foreground">
                  {t('settings.members.inviteDialog.role' + role.charAt(0).toUpperCase() + role.slice(1) + 'Desc')}
                </p>
              </div>
              ...
              >
                {submitting ? (
                  <>
                    ...
                    {t('settings.members.inviteDialog.sending')}
                  </>
                ) : (
