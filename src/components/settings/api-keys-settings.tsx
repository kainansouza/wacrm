...
export function ApiKeysSettings() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(payload.error || t('toast.failedLoadApiKeys'));
        return;
      }
  ...
  async function handleRevoke(key: ApiKey) {
    ...
    try {
      ...
      if (!res.ok) {
        const payload = await res.json().catch(() => ({}));
        toast.error(payload.error || t('toast.failedRevokeKey'));
        return;
      }
      toast.success(t('toast.keyRevoked', { name: key.name }));
...
function CreateKeyDialog({
  ...
  async function handleCreate() {
    ...
    if (!trimmed) {
      toast.error(t('toast.giveKeyName'));
      return;
    }
    setSubmitting(true);
    try {
      ...
      const payload = await res.json().catch(() => ({}));
      if (!res.ok) {
        toast.error(payload.error || t('toast.failedCreateKey'));
        return;
      }
          ...
          <>
            <DialogHeader>
              ...
              <DialogDescription className="text-muted-foreground">
                {t('settings.api.copyWarning')}
              </DialogDescription>
            </DialogHeader>

            <div className="space-y-1.5">
              <Label className="text-muted-foreground">{t('settings.api.title')}</Label>
          ...
          <>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <Label className="text-muted-foreground">{t('settings.api.scopes')}</Label>
