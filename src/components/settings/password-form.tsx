...
export function PasswordForm() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!profile?.email) {
      toast.error(t('toast.cannotChangePassword'));
      return;
    }
    ...
    try {
      ...
      if (signInError) {
        toast.error(t('settings.password.incorrectPassword'));
        return;
      }
      ...
      if (updateError) {
        toast.error(t('toast.passwordUpdateFailed', { message: updateError.message }));
        return;
      }
      setCurrent('');
      setNext('');
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription className="text-muted-foreground">
          {t('settings.password.useAtLeast8')}
        </CardDescription>
      </CardHeader>
