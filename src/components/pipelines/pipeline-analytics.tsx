    ...
    <TooltipProvider>
      <div className="grid grid-cols-2 gap-3 rounded-xl border border-border bg-card/60 p-4 sm:grid-cols-3 xl:grid-cols-6">
        <Metric
          ...
          label={t('pipelines.analyticsTotalDeals')}
          value={String(stats.totalCount)}
          tooltip={t('pipelines.analyticsTotalDealsTooltip')}
        />
        <Metric
          icon={<DollarSign className="h-4 w-4 text-primary" />}
          label={t('pipelines.analyticsPipelineValue')}
          value={formatCurrency(stats.totalValue, defaultCurrency)}
          tooltip={t('pipelines.analyticsPipelineValueTooltip')}
        />
        <Metric
          icon={<Target className="h-4 w-4 text-blue-400" />}
          label={t('pipelines.analyticsAvgDealSize')}
          value={formatCurrency(stats.avgValue, defaultCurrency)}
          tooltip={t('pipelines.analyticsAvgDealSizeTooltip')}
        />
        <Metric
          icon={<TrendingUp className="h-4 w-4 text-purple-400" />}
          label={t('pipelines.analyticsWeightedValue')}
          value={formatCurrency(stats.weightedValue, defaultCurrency)}
          tooltip={t('pipelines.analyticsWeightedValueTooltip')}
        />
        <Metric
          icon={<Trophy className="h-4 w-4 text-primary" />}
          label={t('pipelines.analyticsWonThisMonth')}
          value={String(stats.wonThisMonth)}
          tooltip={t('pipelines.analyticsWonThisMonthTooltip')}
        />
        <Metric
          icon={<XCircle className="h-4 w-4 text-red-400" />}
          label={t('pipelines.analyticsLostThisMonth')}
          value={String(stats.lostThisMonth)}
          tooltip={t('pipelines.analyticsLostThisMonthTooltip')}
        />
      </div>
    </TooltipProvider>
  );
