...
export function PipelineSettings({
  ...
  async function handleSave() {
    ...
    if (renameRes.error || stagesRes.error) {
      toast.error(t('toast.failedSavePipeline'));
      return;
    }
  ...
  async function handleAddStage() {
    ...
    const { data, error } = await supabase
      ...
      .single();
    if (error || !data) {
      toast.error(t('toast.failedAddStage'));
      return;
    }
  ...
  async function handleRemoveStage(stageId: string) {
    ...
    const { count } = await supabase
      ...
      .eq("stage_id", stageId);
    if (count && count > 0) {
      toast.error(t('toast.moveDealsFirst'));
      return;
    }
    const { error } = await supabase
      ...
      .eq("id", stageId);
    if (error) {
      toast.error(t('toast.failedDeleteStage'));
      return;
    }
  ...
  async function handleDeletePipeline() {
    ...
    setDeleting(false);
    if (error) {
      toast.error(t('toast.failedDeletePipeline'));
      return;
    }
    onOpenChange(false);
    onPipelinesChanged();
    toast.success(t('toast.pipelineDeleted'));
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md bg-popover border-border max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-popover-foreground">{t('pipelines.manageTitle')}</DialogTitle>
        </DialogHeader>
        ...
        {showDeleteConfirm ? (
          ...
          <>
            ...
            <DialogFooter className="border-border bg-popover/50">
              ...
              >
                {saving ? t('common.saving') : t('pipelines.saveChanges')}
              </Button>
            </DialogFooter>
          </>
        )}
