...
export function DealForm({
  ...
  async function handleSave() {
    if (!title.trim() || !contactId || !stageId) {
      toast.error(t('toast.titleContactStageRequired'));
      return;
    }
    setSaving(true);
    ...
    if (deal) {
      ...
      if (!accountId) {
        toast.error(t('toast.profileNotLinked'));
        setSaving(false);
        return;
      }
      const { error } = await supabase
  ...
  async function handleStatusChange(status: DealStatus) {
    ...
    if (error) {
      toast.error(t('toast.failedUpdateDealStatus'));
      return;
    }
    toast.success(
      status === "won" ? t('toast.markedAsWon') : status === "lost" ? t('toast.markedAsLost') : t('toast.dealReopened'),
    );
    onOpenChange(false);
    onSaved();
  }
        ...
        <div className="flex h-full flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div className="grid gap-2">
              ...
              >
                <option value="">{t('pipelines.dealForm.contactPlaceholder')}</option>
                {contacts.map((c) => (
            ...
            <div className="grid gap-2">
              <Label className="text-muted-foreground">{t('pipelines.dealForm.closeDateLabel')}</Label>
              <Input
                type="date"
          ...
          <div className="border-t border-border/50 bg-popover/80 p-4">
            <div className="flex gap-2">
              ...
              >
                {saving ? t('common.saving') : deal ? t('pipelines.dealForm.saveChanges') : t('pipelines.dealForm.createDeal')}
              </Button>
            </div>
            ...
            {deal &&
              (confirmDelete ? (
                <div className="mt-3 flex items-center justify-between gap-2 rounded-md border border-red-500/30 bg-red-500/10 px-3 py-2 text-xs">
                  <span className="text-red-300">{t('pipelines.dealForm.confirmDelete')}</span>
