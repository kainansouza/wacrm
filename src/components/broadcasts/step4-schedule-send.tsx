...
export function Step4ScheduleSend({
  ...
  const audienceLabel =
    audience.type === 'all'
      ? t('broadcasts.allContactsLabel')
      : audience.type === 'tags'
        ? t('broadcasts.tagsSelected', { N: audience.tagIds?.length ?? 0 })
        : audience.type === 'csv'
          ? t('broadcasts.csvUpload')
          : t('broadcasts.customLabel');
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-border bg-card/50 p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          <div>
            <p className="text-xs text-muted-foreground">{t('broadcasts.tableTemplate')}</p>
            <p className="text-foreground">{template.name}</p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground">{t('broadcasts.wizardStep2')}</p>
