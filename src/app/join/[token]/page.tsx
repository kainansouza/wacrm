...
export default function JoinPage() {
  ...
  const FAIL_COPY: Record<PeekFail['reason'], { title: string; body: string }> = {
    not_found: {
      title: t('join.failNotFound'),
      body: t('join.failNotFoundBody'),
    },
    used: {
      title: t('join.failAlreadyMember'),
      body: t('join.failAlreadyMemberBody'),
    },
    expired: {
      title: t('join.failExpired'),
      body: t('join.failExpiredBody'),
    },
    server_error: {
      title: t('join.failGeneric'),
      body: t('join.failGenericBody'),
    },
  };
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 409) {
          setConflictMessage(
            payload.error
              ? t(translateSupabaseError(payload.error))
              : t('join.failGenericBody'),
          );
        } else {
          toast.error(
            payload.error
  ...
  const handleSignOutAndRetry = useCallback(async () => {
    ...
    try {
      ...
      toast.error(t('join.errorSignOut'));
      setSigningOut(false);
    }
  }, []);
          ...
          <DialogContent className="bg-popover border-border sm:max-w-md">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2 text-popover-foreground">
                ...
                {t('join.cantJoinWithThisAccount')}
              </DialogTitle>
