...
export default function PipelinesPage() {
  ...
  const handleDealMoved = useCallback(
    async (dealId: string, newStageId: string) => {
      ...
      const { error } = await supabase
        ...
        .eq("id", dealId);
      if (error) {
        toast.error(t('toast.failedMoveDeal'));
        refreshDeals();
      }
    },
  ...
  async function handleCreatePipeline() {
    ...
    if (error || !pipeline) {
      toast.error(t('toast.failedCreatePipeline'));
      setCreating(false);
      return;
    }
    ...
    <div className="space-y-6">
      ...
      {pipelines.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border py-20">
          ...
          <h3 className="mt-4 text-lg font-medium text-foreground">
            {t('pipelines.noPipelinesEmptyTitle')}
          </h3>
          <p className="mt-2 text-sm text-muted-foreground">
            {t('pipelines.noPipelinesEmptyDescription')}
          </p>
          <GatedButton
            canAct={canEditSettings}
            gateReason="create pipelines"
      ...
      <Dialog open={newPipelineOpen} onOpenChange={setNewPipelineOpen}>
        <DialogContent className="sm:max-w-sm bg-popover border-border">
          ...
          <div className="py-2">
            <Label className="text-muted-foreground">{t('pipelines.newPipelineName')}</Label>
            <Input
              value={newPipelineName}
              onChange={(e) => setNewPipelineName(e.target.value)}
              placeholder={t('pipelines.newPipelinePlaceholder')}
          ...
          <DialogFooter className="bg-popover/50 border-border">
            ...
            <Button
              ...
              className="bg-primary text-primary-foreground hover:bg-primary/90"
            >
              {creating ? t('pipelines.creating') : t('pipelines.create')}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
