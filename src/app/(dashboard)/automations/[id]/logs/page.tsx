    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="mt-0.5 text-sm text-muted-foreground">{t('automations.logsTitle')}</p>
        </div>
      </div>
      ...
      {logs.length === 0 ? (
        <div className="flex h-48 flex-col items-center justify-center rounded-xl border border-dashed border-border bg-card/40">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            {t('automations.noExecutionsDescription')}
          </p>
        </div>
      ) : (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="truncate text-xs text-muted-foreground">
                      {log.trigger_event} · {t('automations.steps', { N: log.steps_executed?.length ?? 0 })}
                    </div>
                  </div>
...
function StatusBadge({ status }: { status: AutomationLog["status"] }) {
  const { t } = useT()
  const label: Record<AutomationLog["status"], string> = {
    success: t('automations.success'),
    partial: t('automations.partial'),
